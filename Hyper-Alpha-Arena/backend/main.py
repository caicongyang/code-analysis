...
# 从.env文件加载环境变量，包括数据库连接、API密钥等配置
load_dotenv()
...
# 创建FastAPI应用实例，这是整个后端服务的核心
# Hyper Alpha Arena是一个基于AI的加密货币永续合约交易平台
# 支持双引擎交易模式：AI交易者和程序交易者

# Health check endpoint
# 健康检查端点，用于监控服务状态和版本信息
@app.get("/api/health")
async def health_check():
    ...
    """
    return {
        "status": "healthy",             # 服务状态标识
        "message": "Trading API is running",  # 状态描述信息
        "version": __version__           # 当前版本号
    }

# Manual frontend rebuild endpoint
# 手动前端重构端点，用于开发期间动态重构前端代码
...
async def rebuild_frontend():
    ...
    功能说明：
    - 在开发模式下允许手动重新构建前端
    - 编译React/TypeScript代码到静态文件
    - 将构建结果复制到backend/static目录
    - 支持热更新开发流程
    """
    try:
        build_frontend()  # 调用前端构建函数
