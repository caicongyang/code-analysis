...
class User(Base):
    ...
    设计用于用户身份认证和权限管理，当前项目使用默认用户模式，
    暂不需要用户登录功能，但保留了完整的用户体系以便未来扩展。

    字段说明：
    - id: 用户唯一标识符，主键
    - username: 用户名，必须唯一，用于标识用户
    - email: 邮箱地址，可选，用于通知和找回密码
    - password_hash: 密码哈希，为未来的密码认证预留
    - is_active: 用户激活状态，字符串格式（"true"/"false"）
    - created_at/updated_at: 创建和更新时间戳

    关系映射：
    - accounts: 一对多，用户可拥有多个交易账户
    - auth_sessions: 一对多，用户的认证会话记录
    - subscription: 一对一，用户的订阅信息

    当前使用模式：
    - 系统创建默认用户，所有交易账户都关联到此用户
    - 未来可扩展为多用户系统，支持用户注册和登录
    """
    __tablename__ = "users"
...
class Account(Base):
    ...
    系统的核心数据模型，代表一个独立的交易实体。每个账户可以配置
    独立的AI模型、交易策略和风险参数，支持多账户并行交易。

    设计特点：
    1. 多引擎支持：AI交易者 + 程序交易者
    2. 多交易所集成：原生支持Hyperliquid，可扩展其他交易所
    3. 双环境支持：测试网（纸面交易）+ 主网（真实交易）
    4. 完整风险管理：资金管理、仓位控制、杠杆限制

    账户类型：
    - AI账户：使用大语言模型（GPT、Claude等）进行交易决策
    - 手动账户：支持手动交易，未来可扩展程序交易
