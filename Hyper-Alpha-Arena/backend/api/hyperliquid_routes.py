...
提供Hyperliquid交易所集成的完整管理接口，包括账户配置、
环境切换、余额查询、手动下单等功能。

API端点：
- POST /api/hyperliquid/setup/{account_id}: 配置Hyperliquid账户
- GET /api/hyperliquid/config/{account_id}: 获取账户Hyperliquid配置
- POST /api/hyperliquid/switch-environment: 切换交易环境
- GET /api/hyperliquid/balance/{account_id}: 获取账户余额
- GET /api/hyperliquid/positions/{account_id}: 获取持仓列表
- POST /api/hyperliquid/order: 手动下单（测试用）
- GET /api/hyperliquid/symbols: 获取交易对信息
- PUT /api/hyperliquid/symbols/selected: 更新观察列表

核心功能：
1. 账户配置：设置私钥、杠杆等交易参数
2. 环境管理：testnet/mainnet环境安全切换
3. 余额查询：实时获取账户余额和权益
4. 持仓管理：查询当前持仓和未实现盈亏
5. 手动交易：支持测试环境的手动下单
