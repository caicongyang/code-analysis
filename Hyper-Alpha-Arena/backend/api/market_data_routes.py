...
提供加密货币市场数据的RESTful API接口，包括实时价格、
K线数据、市场状态等基础市场信息。

API端点：
- GET /api/market/price/{symbol}: 获取指定币种的当前价格和市场数据
- GET /api/market/klines/{symbol}: 获取K线数据
- GET /api/market/status/{symbol}: 获取市场交易状态
- GET /api/market/ticker/{symbol}: 获取完整的ticker数据

响应数据：
1. PriceResponse: 价格响应
   - 当前价格、预言机价格
   - 24小时涨跌幅和成交量
   - 持仓量和资金费率

2. KlineResponse: K线响应
   - OHLCV数据
   - 时间戳和格式化时间
   - 涨跌幅和百分比

3. MarketStatusResponse: 市场状态
   - 交易状态（开盘/收盘）
   - 时间戳信息

数据来源：
- 通过market_data服务获取实时数据
- 支持Hyperliquid交易所数据
"""
