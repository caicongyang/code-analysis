...
提供AI提示词回测功能的接口，用于验证提示词修改对交易决策的影响，
支持批量测试和A/B对比分析。

API端点：
- POST /api/prompt-backtest/tasks: 创建回测任务
- GET /api/prompt-backtest/tasks: 获取任务列表
- GET /api/prompt-backtest/tasks/{id}: 获取任务详情
- GET /api/prompt-backtest/tasks/{id}/items: 获取任务测试项列表
- GET /api/prompt-backtest/items/{id}: 获取单个测试项详情
- DELETE /api/prompt-backtest/tasks/{id}: 删除回测任务
