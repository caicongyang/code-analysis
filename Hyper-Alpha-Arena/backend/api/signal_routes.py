...
提供市场信号定义和信号池的完整管理功能，是AI交易触发机制的
核心配置接口。

API端点：
信号定义管理：
- GET /api/signals: 获取所有信号定义和信号池
- POST /api/signals/definitions: 创建新信号定义
- PUT /api/signals/definitions/{id}: 更新信号定义
- DELETE /api/signals/definitions/{id}: 删除信号定义

信号池管理：
- POST /api/signals/pools: 创建新信号池
- PUT /api/signals/pools/{id}: 更新信号池
- DELETE /api/signals/pools/{id}: 删除信号池

触发日志：
- GET /api/signals/trigger-logs: 获取信号触发历史记录

信号定义结构：
- signal_name: 信号名称
- description: 信号描述
- trigger_condition: 触发条件（JSON格式）
  - metric: 监控指标（cvd, oi_delta, funding_rate等）
  - operator: 比较运算符（>, <, >=, <=）
  - threshold: 阈值
  - timeframe: 时间周期
