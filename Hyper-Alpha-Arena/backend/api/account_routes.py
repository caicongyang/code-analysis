...
该模块是前端与账户相关数据交互的核心接口，
支持AI交易者和程序交易者的账户管理需求。
"""
...
def get_db():
    ...
    FastAPI依赖注入函数，为每个请求创建独立的数据库会话。
    使用yield确保请求完成后正确关闭数据库连接，避免连接泄露。
...
def _normalize_bool(value, default=True) -> bool:
    ...
    将各种类型的输入转换为标准布尔值，处理前端传入的
    字符串、数字、空值等多种情况。

    Args:
        value: 待转换的值，可能是字符串、数字、布尔值或None
        default: 当value为None时返回的默认值
    ...
    - bool: 直接返回
    - 数字: 非零为True，零为False
    - 字符串: "true", "1", "yes", "y", "on" 为True（不区分大小写）
    - 其他: 使用Python内置bool()函数转换
    """
    if value is None:
        return default
