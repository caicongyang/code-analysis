...
提供AI交易提示词模板的完整管理功能，包括模板的创建、编辑、
复制、删除以及与账户的绑定关系管理。

API端点：
- GET /api/prompts: 获取所有提示词模板列表
- GET /api/prompts/{id}: 获取指定模板详情
- POST /api/prompts: 创建新的提示词模板
- PUT /api/prompts/{id}: 更新提示词模板内容
- PUT /api/prompts/{id}/name: 更新模板名称
- POST /api/prompts/copy: 复制现有模板
- DELETE /api/prompts/{id}: 删除提示词模板
- GET /api/prompts/bindings: 获取账户-模板绑定关系
- PUT /api/prompts/bindings: 更新绑定关系

模板结构：
- key: 模板唯一标识符
- name: 显示名称
- description: 模板描述
- template_text: 用户可编辑的模板内容
- system_template_text: 系统维护的原始模板

绑定机制：
- 每个AI账户绑定一个提示词模板
- 绑定关系存储在AccountPromptBinding表
- 未绑定的账户使用默认模板
