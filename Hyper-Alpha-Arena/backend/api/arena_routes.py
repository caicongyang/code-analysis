...
为前端仪表盘提供多AI模型交易活动的聚合数据展示，包括已完成交易、
模型对话摘要、综合持仓等信息。

API端点：
- GET /api/arena/trades: 获取最近完成的交易列表
- GET /api/arena/positions: 获取所有账户的当前持仓汇总
- GET /api/arena/chat: 获取AI模型的最近决策对话
- GET /api/arena/summary: 获取整体交易概览统计

核心功能：
1. 交易聚合：汇总多个AI账户的交易活动
2. 持仓展示：实时显示所有账户的持仓情况
3. 对话摘要：展示AI决策的关键信息
4. 性能统计：计算综合收益率和风险指标

数据整合：
- 合并Paper Trading和Hyperliquid交易数据
- 实时获取Hyperliquid账户状态和持仓
- 聚合多账户的统计指标
