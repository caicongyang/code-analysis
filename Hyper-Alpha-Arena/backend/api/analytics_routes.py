...
提供AI交易策略的多维度分析接口，帮助用户深入理解策略表现、
识别优化机会并评估风险特征。

API端点：
- GET /api/analytics/metrics: 获取交易绩效指标
- GET /api/analytics/decisions: 获取决策历史统计
- GET /api/analytics/breakdown: 获取触发类型分析
- GET /api/analytics/performance: 获取时间序列性能数据

分析维度：
1. 绩效指标：总盈亏、胜率、盈亏比、利润因子
2. 触发分析：信号触发vs定时触发的效果对比
3. 决策质量：决策一致性、执行效率分析
4. 风险指标：最大回撤、波动率、夏普比率

数据来源：
- AIDecisionLog: AI决策日志
- ProgramExecutionLog: 程序执行日志
- HyperliquidTrade: Hyperliquid交易记录
- HyperliquidAccountSnapshot: 账户快照
