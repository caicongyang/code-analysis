"""
WebSocket Connection Management and Real-time Data Push
"""
"""
WebSocket连接管理和实时数据推送

提供WebSocket实时通信功能，支持账户数据的实时推送和
前端与后端之间的双向通信。
...
2. 消息类型：
   ...
   - position_update: 持仓变化通知
   - trade_notification: 新交易通知
   - price_update: 价格更新推送
   - arena_asset_update: Arena资产更新

功能特性：
- 自动连接管理：连接建立/断开时自动注册/注销
- 定时快照：为活跃账户启动定时快照任务
- 断线检测：自动清理断开的连接
- 线程安全：支持多线程环境下的消息推送
...
class ConnectionManager:
    ...
    管理所有活跃的WebSocket连接，支持按账户ID分组管理，
    提供消息推送和广播功能。

    核心属性：
    - active_connections: 按账户ID组织的连接字典
    - _loop: 事件循环引用（用于跨线程推送）

    主要方法：
    - register(): 注册新连接
    - unregister(): 注销连接
    - send_to_account(): 向特定账户推送消息
    - broadcast_to_all(): 向所有连接广播消息

    连接生命周期：
    1. connect: WebSocket连接建立
    2. register: 注册到特定账户
    3. 消息推送: 持续推送数据
    4. unregister: 断开时注销
    """
