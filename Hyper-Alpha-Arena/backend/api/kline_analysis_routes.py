...
提供AI驱动的K线图表分析功能，允许用户将K线数据和技术指标
发送给AI模型进行智能分析。

API端点：
- POST /api/klines/ai-analysis: 请求AI分析K线图表
- GET /api/klines/ai-analysis/history: 获取分析历史记录
- GET /api/klines/ai-analysis/{id}: 获取特定分析详情

请求参数：
- account_id: 使用的AI账户（决定AI模型配置）
- symbol: 交易标的
- period: K线周期
- klines: K线数据数组
- indicators: 技术指标数据
- market_data: 当前市场数据（价格、成交量等）
- positions: 当前持仓信息
- user_message: 用户附加问题（可选）
