...
提供订单的创建、查询和取消功能，支持Paper Trading模式下的
市价单和限价单操作。

API端点：
- POST /api/orders/create: 创建新订单
- GET /api/orders/pending: 获取待执行订单列表
- POST /api/orders/cancel/{id}: 取消指定订单
- POST /api/orders/execute/{id}: 手动执行订单
- POST /api/orders/process-all: 处理所有待执行订单
...
认证方式：
1. session_token: 推荐的认证方式
2. username + password: 传统认证方式（已弃用）

执行流程：
1. 验证用户权限
2. 检查账户余额/持仓
3. 创建订单记录
4. 市价单立即执行，限价单加入待执行队列
5. 返回订单状态

注意：此接口主要用于Paper Trading测试
Hyperliquid实盘交易使用专门的API
"""
