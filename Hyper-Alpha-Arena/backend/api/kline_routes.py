"""
K-line Data Management API Routes
"""
"""
K线数据管理API路由

提供K线数据的查询、补漏和管理功能，支持多交易所、多周期的
K线数据访问和历史数据补全。

API端点：
- GET /api/klines/coverage: 查询K线数据覆盖范围
- GET /api/klines/data: 获取指定范围的K线数据
- POST /api/klines/backfill: 创建历史数据补漏任务
- GET /api/klines/backfill/tasks: 获取补漏任务列表
- GET /api/klines/backfill/tasks/{id}: 获取补漏任务详情

核心功能：
1. 数据覆盖查询：查看已收集数据的时间范围
2. K线数据获取：按交易对、周期、时间范围查询
3. 历史补漏：异步补充缺失的历史K线数据
4. 任务管理：监控补漏任务的执行进度
...
数据模型：
- BackfillRequest: 补漏任务请求
- BackfillTaskResponse: 任务状态响应
- CoverageResponse: 数据覆盖范围响应
"""
