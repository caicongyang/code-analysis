...
"""
"""
账户管理API路由 - 处理交易账户的增删改查操作

提供完整的账户生命周期管理接口，包括创建、查询、更新和停用账户。
支持AI交易账户和手动交易账户的统一管理。

API端点：
- GET /api/accounts/: 获取当前用户的所有账户列表
- POST /api/accounts/: 创建新的交易账户
- GET /api/accounts/{id}: 获取指定账户详情
- PUT /api/accounts/{id}: 更新账户配置
- DELETE /api/accounts/{id}: 停用账户（软删除）

功能特性：
1. 用户认证：基于session token的访问控制
2. 账户隔离：用户只能访问自己的账户
3. Hyperliquid集成：支持实时获取交易所账户余额
4. 灵活配置：支持AI模型、API密钥等配置

数据模型：
- AccountCreate: 创建账户请求
- AccountUpdate: 更新账户请求
- AccountOut: 账户响应数据
