...
基于大语言模型的智能交易策略诊断和归因分析系统。
通过自然语言对话的方式，帮助用户深度分析交易表现，识别问题并提供改进建议。

核心功能：
1. 智能对话分析：支持自然语言询问交易表现问题
2. 函数调用能力：AI可主动查询交易数据、性能指标、历史记录
3. 策略诊断：深入分析策略的盈亏来源和风险特征
4. 改进建议：基于数据分析提供具体的优化方向
5. 多环境支持：支持测试网和主网交易数据的独立分析

AI角色定位：
- **策略诊断医生**：像医生诊断病情一样分析交易问题
- **数据分析专家**：从海量交易数据中提取有价值的洞察
- **投资顾问**：提供专业的策略优化建议

技术架构：
- LLM对话引擎：支持多轮对话和上下文理解
- Function Calling：AI可调用专门的数据查询函数
- 环境隔离：测试网和主网数据完全分离分析
- 会话持久化：保存对话历史供后续参考

支持的分析类型：
1. **性能归因分析**：分析盈亏来源，识别贡献最大的交易
2. **风险评估**：分析最大回撤、波动率、夏普比率等
3. **交易行为分析**：分析交易频率、持仓时间、胜率等
4. **策略对比**：比较不同策略或时间段的表现
5. **市场环境影响**：分析不同市场条件下的策略适应性

应用价值：
- **降低分析门槛**：无需复杂的数据分析技能
- **提升分析深度**：AI能发现人类容易忽略的模式
- **加速优化迭代**：快速识别问题并获得改进方向
- **知识积累**：对话记录形成策略优化的知识库
"""
...
# 该系统提示词设计了一个专业的交易策略诊断医生角色，具有以下特点：
# 1. 专业身份：定位为交易策略诊断医生，建立权威性和专业性
# 2. 环境安全：强制要求确认分析环境（测试网/主网），避免数据混淆
# 3. 用户友好：支持通过账户名称而非ID来识别账户，提升用户体验
# 4. 工作流程：明确的分析流程，确保数据查询的准确性和完整性
# 5. 函数调用：指导AI正确使用可用的数据查询功能
