...
为Hyper Alpha Arena前端提供高性能的资产曲线数据，用于绘制账户
净值变化图表。采用SQL级别的数据聚合和进程内缓存实现高效查询。

核心功能：
1. 资产曲线生成：基于历史快照数据生成时间序列资产曲线
2. 时间聚合：支持5分钟、1小时、1天三种时间粒度聚合
3. 多账户对比：同时返回多个账户的资产曲线用于性能比较
4. 缓存优化：进程内缓存减少重复查询开销

技术架构：
- SQL聚合：在数据库层面进行时间桶聚合，减少数据传输量
- 智能缓存：基于最新快照ID的缓存失效策略
- 双数据源：支持Paper Trading快照和Hyperliquid快照两种数据源
- 线程安全：使用锁保护缓存的并发访问

时间桶策略：
- 5m: 每5分钟取一个数据点，适合短期精细分析
- 1h: 每小时取一个数据点，适合日内趋势查看
- 1d: 每天取一个数据点，适合长期趋势分析
