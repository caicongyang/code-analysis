...
通过对话式AI助手帮助用户创建市场信号定义。支持Function Calling
让AI查询真实市场数据，实现数据驱动的信号设计。

核心功能：
1. 引导式对话：通过问答了解用户的交易偏好和需求
2. 数据分析：调用工具分析历史市场数据
3. 阈值优化：基于历史数据推荐合理的信号阈值
4. 回测验证：测试信号组合的触发频率

可用工具（Function Calling）：
- get_indicators_batch: 批量获取多个指标的统计数据（百分位数）
- predict_signal_combination: 预测信号组合的触发频率
- get_kline_context: 获取K线上下文验证触发质量

工作流程：
1. 了解用户需求（交易方向、信号类型、触发频率）
2. 分析相关指标的历史分布
3. 设定初步阈值并测试组合效果
4. 根据回测结果调整优化
5. 输出最终信号池配置
...
可用指标：
- oi_delta_percent: 持仓量变化百分比
- funding_rate: 资金费率变化（基点）
- cvd: 累积成交量差值
- taker_ratio: 主动买卖比率
- depth_ratio: 订单簿深度比率
