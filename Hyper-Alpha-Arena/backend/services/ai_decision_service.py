...
"""
"""
AI决策服务 - 处理交易决策的AI模型API调用

这是Hyper Alpha Arena的核心AI引擎，负责：
1. 与各种LLM API（OpenAI、Claude、Deepseek等）通信
2. 构建包含市场数据、技术指标、新闻等的完整上下文
3. 解析AI模型返回的交易决策
4. 记录决策日志用于策略分析和优化
5. 处理API调用的容错和重试机制
...
核心功能：
- 智能上下文构建：整合价格、指标、持仓、新闻等多维数据
- 多模型支持：统一接口支持不同AI模型
- 决策解析：从自然语言回复中提取结构化交易指令
- 风险控制：集成资金管理和仓位限制
- 性能监控：记录响应时间和成功率统计
"""
import logging
...
# 应跳过的演示模式API密钥
DEMO_API_KEYS = {
    "default-key-please-update-in-settings",  # 默认提示用户更新的密钥
    "default",                                # 通用默认值
    "",                                       # 空字符串
    None                                      # 空值
}
# 这些API密钥被识别为无效的演示密钥，系统会拒绝使用它们进行真实API调用
# 目的是避免用户忘记配置有效密钥而导致的API调用失败
...
# 支持的交易标的符号映射表
# 将简短的交易符号映射为完整的货币名称，用于：
# 1. AI上下文构建：向AI模型提供更完整的货币信息
# 2. 用户界面显示：展示完整的货币名称
# 3. 新闻关联：匹配相关的市场新闻和分析
# 4. 多语言支持：便于后续的国际化扩展
# 注意：这里的符号需要与Hyperliquid交易所的符号保持一致
...
class SafeDict(dict):
    ...
    扩展标准字典，当访问不存在的键时返回"N/A"而不是抛出异常。
    这在构建AI上下文时特别有用，可以避免因缺失数据导致的程序崩溃。
...
def _format_currency(value: Optional[float], precision: int = 2, default: str = "N/A") -> str:
    ...
    将数值格式化为易读的货币字符串，处理各种边界情况。
    为AI上下文提供一致的数据表示格式。

    Args:
        value: 待格式化的数值，可能为None
        precision: 小数位精度，默认2位
        default: 当值无效时返回的默认字符串

    Returns:
        str: 格式化后的货币字符串，如"$1,234.56"或默认值
