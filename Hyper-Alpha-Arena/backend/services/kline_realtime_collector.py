"""
K-line Realtime Collection Service - Collects current K-line data every minute
"""
"""
K线实时采集服务

每分钟定时采集当前K线数据，确保系统拥有最新的价格和成交量信息。
同时检测历史数据缺失并自动触发补漏任务。

核心功能：
1. 实时采集：每分钟结束后采集多个周期的K线数据
2. 多周期支持：同时采集1m、3m、5m、15m、30m、1h周期数据
3. 缺失检测：定期检测历史数据缺失并创建补漏任务
4. 多交易对：支持同时采集多个交易对的K线数据

采集周期：
- 1m: 1分钟K线，最细粒度的价格数据
- 3m/5m/15m/30m: 中等周期，适合日内交易分析
- 1h: 小时K线，适合趋势判断
...
class KlineRealtimeCollector:
    """K-line realtime collection service"""
    """
    K线实时采集服务类

    异步服务，定期采集K线数据并检测数据缺失。
    作为系统K线数据的主要来源，确保数据的及时性和完整性。

    核心属性：
    - default_symbols: 默认采集的交易对（BTC、ETH、SOL等）
    - periods: 采集的K线周期列表
    - running: 服务运行状态标志

    任务管理：
    - collection_task: 实时采集循环任务
    - gap_detection_task: 缺失检测循环任务

    采集策略：
    - 等待每分钟结束后5秒开始采集，确保K线数据完整
    - 多交易对和多周期并行采集
    - 失败重试机制保证数据可靠性
    """
