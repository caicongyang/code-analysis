...
"""
"""
Hyperliquid市场数据服务 - 基于CCXT库

通过CCXT统一接口获取Hyperliquid交易所的市场数据，
包括实时价格、K线数据、交易对信息等。

核心功能：
1. 实时价格获取：获取指定交易对的最新价格
2. K线数据获取：获取历史和实时K线数据
3. 交易对管理：获取和格式化交易对符号
4. 环境适配：支持测试网和主网环境

技术特点：
- CCXT集成：使用CCXT库统一处理交易所API差异
- 频率限制：内置速率限制防止API超限
- 市场缓存：预加载市场信息减少API延迟
- HIP3禁用：跳过HIP3 DEX市场，仅使用永续合约市场
