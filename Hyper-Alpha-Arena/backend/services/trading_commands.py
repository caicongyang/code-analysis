...
这是Hyper Alpha Arena交易系统的核心执行引擎，负责将AI交易决策
转换为具体的订单操作，并与各种交易所进行交互。

核心功能：
1. 订单执行：将AI决策转换为实际的买卖订单
2. 价格保护：确保订单价格在合理范围内，防止异常交易
3. 风险控制：实施资金管理和仓位限制
4. 多交易所支持：统一接口支持不同的交易执行后端
5. 交易记录：完整记录所有交易操作供后续分析

设计架构：
- **命令模式**：将交易决策封装为可执行的命令对象
- **策略模式**：支持不同交易所的执行策略
- **工厂模式**：根据配置创建相应的交易客户端
- **观察者模式**：交易执行状态的事件通知

支持的交易类型：
1. **AI驱动交易**：基于大语言模型决策的自动交易
2. **程序化交易**：基于代码逻辑的策略交易
3. **手动交易**：用户手动发起的交易指令
4. **风险管理交易**：自动止损止盈交易

交易执行流程：
1. 决策接收：接收来自AI或程序的交易决策
2. 参数验证：验证交易参数的合法性和安全性
3. 价格检查：确保交易价格在合理范围内
4. 资金检查：验证账户资金是否足够执行交易
5. 订单提交：向交易所提交订单请求
6. 状态监控：监控订单执行状态并处理异常
7. 记录更新：更新账户状态和交易历史

安全特性：
- **价格偏差限制**：防止异常价格导致的巨额损失
- **资金管理**：严格控制单笔交易金额和总仓位
- **订单验证**：多重验证确保订单参数正确
- **异常处理**：完善的错误处理和回滚机制

性能优化：
- **异步执行**：支持异步订单提交，提高并发性能
- **批量操作**：支持批量订单处理，减少延迟
- **连接池**：维护交易所连接池，提高执行效率
- **缓存机制**：缓存常用数据，减少查询开销
"""
...
# AI交易支持的标的列表
# 这些是AI交易者可以交易的加密货币标的，选择标准：
# - 流动性充足：确保交易时滑点较小
# - 波动性适中：既有盈利机会又不过于风险
# - 市场认知度高：新闻和基本面信息丰富，有利于AI分析
# - 技术指标有效：历史数据充分，技术分析可靠

ORACLE_PRICE_DEVIATION_LIMIT_PERCENT = 1.0
# 预言机价格偏差限制（百分比）
# 防范价格异常导致的意外交易损失：
# - 限制交易价格与当前市价的偏差不超过1%
# - 避免因网络延迟、数据错误等导致的异常价格交易
# - 平衡交易灵活性和风险控制的需求
# - 可根据市场波动情况调整此参数
...
def _enforce_price_bounds(
    *,
    ...
    将请求的交易价格限制在当前市价的±1%范围内，防止异常价格导致的
    意外损失。这是交易系统的重要安全机制。

    Args:
        symbol: 交易标的符号（如"BTC"、"ETH"）
        account_name: 账户名称，用于日志记录
        operation: 交易操作类型（buy/sell/close等）
        current_price: 当前市场价格（预言机价格）
        requested_price: AI或策略请求的交易价格

    Returns:
        Tuple[float, float, bool]: (调整后价格, 偏差百分比, 是否被调整)

    价格调整逻辑：
    1. 计算允许的价格范围：current_price ± 1%
    2. 如果请求价格超出范围，则调整到边界值
    3. 记录调整信息到日志，供后续分析
    4. 返回最终价格和调整情况
