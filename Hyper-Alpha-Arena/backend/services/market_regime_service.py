...
基于多维市场数据的智能市场制度分类系统，是Hyper Alpha Arena交易决策的核心组件之一。
通过分析价格、成交量、持仓量、订单流等多个维度的数据，将复杂的市场状态分类为7种标准制度。

7种市场制度详解：
1. **Stop Hunt（猎杀止损）**：
   - 特征：价格突破关键水平后快速反转
   - 市场行为：大资金故意触发散户止损，然后反向操作
   - 交易信号：短期剧烈波动后的反转机会

2. **Absorption（吸收）**：
   - 特征：强劲资金流入但价格变动有限
   - 市场行为：大单被市场有效吸收，显示强支撑/阻力
   - 交易信号：可能的反转信号，需要观察后续突破

3. **Breakout（突破）**：
   - 特征：趋势启动，各项指标协调一致
   - 市场行为：价格突破关键阻力/支撑，伴随成交量放大
   - 交易信号：趋势跟随的最佳入场时机

4. **Continuation（延续）**：
   - 特征：趋势持续，动量保持
   - 市场行为：现有趋势稳定延续，无明显反转迹象
   - 交易信号：趋势持有信号，避免逆势操作
...
技术指标定义：
- **cvd_ratio**: CVD / 总成交额（非z-score标准化）
- **taker_ratio**: ln(买盘成交额 / 卖盘成交额) - 对数变换保持对称性
- **oi_delta**: 持仓量变化百分比
- **price_atr**: 价格变化 / 平均真实波幅（ATR）
- **rsi**: 14周期相对强弱指标

算法特点：
- **多维分析**：综合价格、成交量、持仓量、订单流等多个维度
- **实时计算**：支持实时和历史数据的制度分类
- **置信度评估**：为每个分类结果提供置信度评分
- **参数可配置**：支持不同市场环境下的参数调优

应用价值：
1. **交易时机优化**：在合适的市场制度下执行相应策略
2. **风险控制**：在高风险制度（如陷阱）下减少仓位
3. **策略适应性**：不同制度下采用不同的交易逻辑
4. **市场理解**：帮助交易者深度理解市场微观结构
"""
...
def fetch_ohlc_from_flow(
    ...
    基于15秒粒度的市场交易聚合数据构建"模拟K线"，用于实时市场制度分类。
    这种方法比传统K线数据更及时，能够捕捉更细微的市场变化。

    Args:
        db: 数据库会话对象
        symbol: 交易标的符号（如"BTC"、"ETH"）
        period: 时间周期（"1m", "5m", "15m", "1h"）
        limit: 返回的K线数量限制，默认15根
        current_time_ms: 锚点时间戳（毫秒），默认使用当前时间

    Returns:
        List[Dict]: OHLC数据列表，每个元素包含：
                   - timestamp: 时间戳（毫秒）
                   - open: 开盘价
                   - high: 最高价
                   - low: 最低价
    ...
    实现原理：
    1. 时间锚点：以current_time_ms为基准点向前推算
    2. 时间窗口：每个K线周期对应一个时间窗口
    3. 数据聚合：在每个窗口内聚合15秒数据为OHLC
    4. 滑动窗口：向前滑动构建历史K线序列
    ...
    应用场景：
    - 实时制度分类：为当前市场状态分类提供数据
    - 技术指标计算：为RSI、ATR等指标提供价格数据
    - 回测验证：验证历史时刻的制度分类准确性
    """
