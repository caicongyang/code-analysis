...
异步执行AI提示词回测任务，验证提示词修改对交易决策的影响。
支持并行处理和实时进度更新。

核心功能：
1. 并行执行：使用线程池并行调用LLM，提高回测效率
2. 实时进度：每个测试项完成后立即更新进度
3. 结果解析：解析LLM响应，提取交易决策
4. 一致性分析：比较原始决策和修改后的决策差异
...
- A/B测试：比较不同提示词版本的效果
- 策略调优：找到最优的提示词表达方式
- 质量保证：确保提示词修改不会产生意外行为

性能配置：
- MAX_WORKERS: 最大并行工作线程数（默认20）
- 适当的并行度可以显著减少回测时间
"""
