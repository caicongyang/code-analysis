...
"""
"""
Hyperliquid交易客户端 - 真实交易执行与环境隔离

这是Hyper Alpha Arena平台与Hyperliquid DEX（去中心化永续合约交易所）
交互的核心模块，提供完整的交易功能和风险管理。

核心功能：
1. 环境隔离：严格区分测试网（testnet）和主网（mainnet）环境
2. 账户管理：实时同步账户余额、保证金、持仓等状态信息
3. 订单执行：支持市价单、限价单、止损止盈等多种订单类型
4. 杠杆交易：支持1-50倍杠杆的永续合约交易
5. 风险控制：实时监控保证金率、强平价格等风险指标

技术特性：
- EIP-712签名：使用以太坊标准的类型化数据签名机制
- 精度控制：使用Decimal确保财务计算的精确性
- 错误处理：完善的异常处理和重试机制
- 缓存优化：本地缓存减少API调用频率
- 审计日志：记录所有交易操作供后续分析
...
Hyperliquid简介：
Hyperliquid是建立在以太坊L1上的去中心化永续合约交易所，
提供接近中心化交易所的交易体验，同时保持去中心化的优势。
支持50+主流加密货币的永续合约交易，具有深度流动性和低滑点。
"""
import logging
import time
import json
...
getcontext().prec = 28  # 设置Decimal的全局精度为28位，确保金融计算的准确性
# 高精度计算设置：
# - 28位精度足以处理大多数加密货币价格和数量计算
# - 避免浮点数精度问题导致的交易错误
# - 符合DeFi协议对精度的要求
