...
管理Hyperliquid交易所支持的交易对信息，包括可用交易对的获取、
用户观察列表的配置、以及与其他服务的符号同步。

核心功能：
1. 交易对获取：从Hyperliquid Meta API获取可交易的交易对列表
2. 观察列表管理：用户自定义观察列表（最多10个交易对）
3. 符号格式化：统一交易对符号的格式和显示名称
4. 定期刷新：自动更新交易对信息，适应交易所变化
5. 数据流同步：确保市场数据流订阅正确的交易对

数据存储：
- SystemConfig表存储可用交易对列表
- SystemConfig表存储用户选择的观察列表
- JSON格式序列化交易对信息
...
配置常量：
- MAX_WATCHLIST_SYMBOLS: 观察列表最大数量（10个）
- DEFAULT_SYMBOLS: 默认交易对列表（BTC）
"""
