...
为AI提示词注入提供格式化的市场流量数据。与返回时间序列的
图表API不同，本服务返回当前值、历史值和比较上下文。

支持的变量：
- {SYMBOL}_CVD_{PERIOD}: 累积成交量差值（CVD）
  买方主动成交量 - 卖方主动成交量，正值表示买盘主导

- {SYMBOL}_TAKER_{PERIOD}: 主动成交量和比率
  主动买入/卖出的成交量及其比率

- {SYMBOL}_OI_{PERIOD}: 持仓量（Open Interest）
  市场总未平仓合约数量，反映市场参与度

- {SYMBOL}_FUNDING_{PERIOD}: 资金费率
  永续合约的资金费率，正值表示多头支付空头

- {SYMBOL}_DEPTH_{PERIOD}: 订单簿深度比率
  买盘深度/卖盘深度，反映订单簿的偏向性
