...
"""
"""
AI提示词生成服务 - 处理AI驱动的交易提示词生成

通过对话式AI助手帮助用户创建和优化交易策略的提示词模板。
支持Function Calling让AI查询变量参考、验证提示词和预览效果。

核心功能：
1. 对话式创建：通过自然语言对话生成提示词
2. 变量注入：支持动态变量（价格、指标、持仓等）
3. 提示词验证：检查语法和变量引用的正确性
4. 效果预览：展示提示词填充变量后的实际效果

可用工具（Function Calling）：
- get_variables_reference: 获取可用变量的完整参考文档
- validate_prompt: 验证提示词的语法和变量引用
- preview_prompt: 预览提示词填充实际数据后的效果
- get_signal_pools: 获取信号池配置（共享工具）
- run_signal_backtest: 运行信号回测（共享工具）

技术架构：
- 与ai_program_service.py保持架构一致性
- 使用相同的重试配置和错误处理逻辑
- 支持流式响应和实时进度展示
- 会话管理和消息持久化
